...
class LiveMessageStream{
  ...
  LiveMessageStream({required this.streamToken,required this.roomId,required this.uid,required this.host,required this.port});
  late Socket socket;
  bool heartBeat=true;
        ...
        try{
          switch(header.protocolVer){
            ...
            case 2:
              ...
              break;
            case 3:
              decompressedData=const BrotliDecoder().convert(data.sublist(0x10));
  ...
  void _processingData(List<int> data){
    try{
      ...
      var msgBody=utf8.decode(data.sublist(subHeader.headerSize,subHeader.totalSize));
      for (var f in eventListeners) {f(jsonDecode(msgBody));}
  ...
  void _heartBeat()async{
    ...
    while(heartBeat){
      ...
      //发送心跳包
      var package=HeartbeatPackage(header: PackageHeader(totalSize: 0, headerSize: 0, protocolVer: 1, operationCode: 2, seq: heartBeatCount));
      try{
